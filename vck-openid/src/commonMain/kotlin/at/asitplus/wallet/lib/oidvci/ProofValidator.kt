...
class ProofValidator(
    ...
    private suspend fun JwsSigned<KeyAttestationJwt>.validateAttestationProof(): Collection<CryptoPublicKey> {
        ...
        val attestationStatus =
            runCatching { Json.decodeFromJsonElement<StatusListInfo>(payload.status!!["status_list"]!!) }.getOrNull()
                ?.let { statusListInfo ->
                    val uri = statusListInfo.uri
                    val statusListTokenPayload =
                        runCatching { statusListTokenResolver(statusListUrl = uri) }.getOrNull()?.let {
                            it.validate(revocationListInfo = statusListInfo, isInstantInThePast = { it < clock.now() })
                        }
                    val statusList = statusListTokenPayload?.getOrNull()?.revocationList as StatusList
                    statusList.toView(DefaultZlibService()).getOrNull(statusListInfo.index)
                }
